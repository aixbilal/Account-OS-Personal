...
const priorityVisuals: Record<string, {
  ...
  // Chrome and Slack ship no `simple-icons` path in the installed package
  // version (verified absent, same as Microsoft/LinkedIn above), so these
  // are hand-built local recognition marks rendered directly in
  // ServiceIdentity.tsx (.chrome-mark / .slack-mark), not SVG paths.
  chrome: localVisual("#1A73E8", "#E8F0FE"),
  slack: localVisual("#4A154B", "#F3E9F2"),
};
